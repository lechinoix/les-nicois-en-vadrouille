...
<div id={galleryName}>
	{#each pictures as picture}
		<a data-lg-size={`${picture.width}-${picture.height}`} data-src="{formatAssetUrl(picture.formats.large.url)}">
			<img alt={picture.alternativeText} src={formatAssetUrl(picture.formats.small.url)} class="image__inpage" />
		</a>
	{/each}
</div>
